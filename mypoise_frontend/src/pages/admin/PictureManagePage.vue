  ...
  <div id="pictureManagePage">
    ...
    <a-form layout="inline" :model="searchParams" @finish="doSearch">
      ...
      <a-form-item label="类型">
        <a-input v-model:value="searchParams.categoryName" placeholder="请输入类型" allow-clear />
      </a-form-item>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'tags'">
          <a-space wrap>
            <a-tag v-for="tag in record.tagNames || '[]'" :key="tag">
              {{ tag }}
            </a-tag>
          </a-space>
        </template>
        ...
        <!--todo 可以使用模态框 动态指定审核信息，https://antdv.com/components/modal-cn。。。。。-->
        <!-- todo 增加按钮确认 https://antdv.com/components/popconfirm-cn-->
...
import DeleteRequest = API.DeleteRequest;
...
const doDelete = async (id: string) => {
  ...
  const res = await deletePictureUsingPost({ id } as DeleteRequest)
  if (res.data.code === 0) {
